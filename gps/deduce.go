...
func (m gitoliteDeducer) deduceSource(path string, u *url.URL) (maybeSources, error) {
...
func (m golangDeducer) deduceSource(path string, u *url.URL) (maybeSources, error) {
	...
	if (os.Getenv(uber.TurnOffUberDeduceLogicEnv) == "" && !inTest) ||
		...
		if err == nil {
			return maybeSources{maybeGitoliteSource{url: golangUrl, gpath: gpath, remote: remote, gitoliteURL: gitoliteURL}}, nil
		}
	}
...
func (m gopkginDeducer) deduceSource(p string, u *url.URL) (maybeSources, error) {
	...
	if (os.Getenv(uber.TurnOffUberDeduceLogicEnv) == "" && !inTest) ||
		...
		if err == nil {
			u = uberUrl
			return maybeSources{maybeGopkginSource{
				opath:    v[1],
				url:      u,
				major:    major,
				unstable: unstable,
