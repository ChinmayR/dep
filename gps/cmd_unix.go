...
func (c cmd) CombinedOutput() ([]byte, error) {
		...
		case <-c.ctx.Done():
			if err := c.Cmd.Process.Signal(os.Interrupt); err != nil {
				...
				defer time.AfterFunc(time.Minute, func() {
					uber.UberLogger.Printf("timed out command %v at %v, out: %v", c.Cmd.Args, c.Cmd.Dir, string(b.Bytes()))
					_ = c.Cmd.Process.Kill()
				}).Stop()
				<-waitDone
			}
	...
	if err := c.Cmd.Wait(); err != nil {
		uber.UberLogger.Printf("error executing command %v at %v, err:%v, out: %v", c.Cmd.Args, c.Cmd.Dir, err, string(b.Bytes()))
		return b.Bytes(), err
	}
