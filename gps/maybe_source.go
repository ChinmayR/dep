...
func (m maybeGitoliteSource) try(ctx context.Context, cachedir string) (source, error) {
	...
	path := sourceCachePath(cachedir, m.url.String())
...
func (m maybeGitoliteSource) URL() *url.URL {
	// return the local mirrored URL even if the underlying source might be pointing to the upstream remote URL
	return m.url
}

func (m maybeGitoliteSource) String() string {
	// return the local mirrored URL even if the underlying source might be pointing to the upstream remote URL
