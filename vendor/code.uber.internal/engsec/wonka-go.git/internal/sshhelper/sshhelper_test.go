...
func TestUsshHostCertWhenInvalidCertPathShouldError(t *testing.T) {
	f, err := ioutil.TempFile("", "usshHostCert")
	require.NoError(t, err, "should not error creating tempfile")
	defer os.Remove(f.Name())

	// empty host cert path
	f.WriteString("HostCertificate \nHostKey \n")
	_, _, err = UsshHostCert(zap.NewNop(), f.Name())
	require.Error(t, err, "should not allow empty host cert path")

	f.Truncate(0)
	f.WriteString("HostCertificate invalid\nHostKey invalid\n")
	_, _, err = UsshHostCert(zap.NewNop(), f.Name())
	require.Error(t, err, "should not allow invalid cert path")

}

func TestUsshHostCertWhenInvalidCertShouldError(t *testing.T) {
	f, err := ioutil.TempFile("", "usshHostCert")
	require.NoError(t, err, "should not error creating tempfile")
	defer os.Remove(f.Name())
...
func TestUsshHostCertWhenInvalidKeyShouldError(t *testing.T) {
	withSSHConfig(t, "invalidKey", func(configPath string) {
		// Delete the key
		contents, err := ioutil.ReadFile(configPath)
		require.NoError(t, err, "should not error reading file")

		idx := strings.Index(string(contents), "\n")
		hostCertPath := contents[:idx]
		ioutil.WriteFile(configPath, hostCertPath, 0644)

		_, _, err = UsshHostCert(zap.NewNop(), configPath)
		require.Error(t, err, "should not permit invalid key")
	})
}

func TestUsshHostCertWhenInvalidConfigPathShouldError(t *testing.T) {
	_, _, err := UsshHostCert(zap.NewNop(), "")
	require.Error(t, err, "should not permit invalid config path")
}
...
func TestPrivKeyFromPathEmptyPathShouldError(t *testing.T) {
...
func TestPrivKeyFromPathInvalidPathShouldError(t *testing.T) {
	...
	_, err = privKeyFromPath(&cert, []string{"ssh_host_ed25519_key"})
	require.Error(t, err, "empty path should fail")
}
