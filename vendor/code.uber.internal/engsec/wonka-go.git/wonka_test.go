...
func setEnvWithCleanup(envvar, val string) (restore func()) {
	prevVal := os.Getenv(envvar)
	os.Setenv(envvar, val)
	return func() {
		os.Setenv(envvar, prevVal)
	}
}
	...
	for idx, m := range initVars {
		WithTempDir(func(dir string) {
			...
			case "env":
				...
				require.NoError(t, err, "writing private key shouldn't fail: %v", e)
				defer setEnvWithCleanup("WONKA_CLIENT_CERT", certPath)()
				...
				require.NoError(t, err, "writing private key shouldn't fail: %v", e)
				defer setEnvWithCleanup("WONKA_CLIENT_KEY", eccKeyPath)()
			case "rsa":
			...
			if m.errString != "" {
				require.Error(t, err, "[%d] should fail", idx)
				require.Contains(t, err.Error(), m.errString, "[%d] wrong error returned", idx)
			} else {
				require.NoError(t, err, "[%d] should not error", idx)
			}
		})
	}
}
...
func TestWonkaMasterECCKey(t *testing.T) {
	...
	for idx, m := range keyVars {
		oldECCPub := os.Getenv("WONKA_MASTER_ECC_PUB")
		os.Setenv("WONKA_MASTER_ECC_PUB", wonka.ECCPUB)

		if m.compressed {
			os.Setenv("WONKA_MASTER_ECC_PUB", wonka.ECCPUB)
			if m.badCompressed {
				os.Setenv("WONKA_MASTER_ECC_PUB", "foober")
			}
		}
		...
		os.Setenv("WONKA_MASTER_ECC_PUB", oldECCPub)
	}
}
...
func TestSettingMultipleMasterKeys(t *testing.T) {
	...
	oldPubkey := os.Getenv("WONKA_MASTER_ECC_PUB")
	defer os.Setenv("WONKA_MASTER_ECC_PUB", oldPubkey)

	os.Setenv("WONKA_MASTER_ECC_PUB", strings.Join(compressedKeys, ","))
	err := wonka.InitWonkaMasterECC()
