...
def coverage_is_sufficient(path_root, config):
    ...
    # We're going to report lines that don't have coverage if a high amount of
    # coverage is requested and we don't have enough.
    uncovered_line_list = None
    ...
    for class_to_check in root.iter('class'):
        filename = class_to_check.get('filename')
        for method in class_to_check.iter('method'):
            for line in method.iter('line'):
                if line.get('hits') == '0':
                    uncovered_lines += 1
                    if uncovered_line_list is not None:
                        line_number = line.get('number')
                        uncovered_line_list.append((filename, line_number))
                else:
                    covered_lines += 1
    ...
    if actual_percent < min_percent:
        ...
        report_uncovered_lines(uncovered_line_list)
        return False
    return True


def report_uncovered_lines(uncovered_line_list):
    """Prints a meaningful error message to stderr"""
    logging.error('Run `make test-html && open coverage.html` to view a '
                  'coverage report locally')
    if not uncovered_line_list:
        return
    logging.error('Missing coverage from lines:')
    for filename, line_number in uncovered_line_list:
        logging.error('%s line %s', filename, line_number)
