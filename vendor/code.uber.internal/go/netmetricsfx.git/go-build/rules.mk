...
# If the vendor dir doesn't exist and they don't have a glide.yaml, or have a Gopkg.toml,
# then assume they're using Godeps (cringe)
ifeq ($(shell ( [ -f $(DEP_TOML) ]) && echo y),y)
	# Email dep-support-group@uber for issues regarding dep
	DEP_MANAGER ?= Dep
else ifeq ($(shell ( [ -f $(GLIDE_YAML) ] ) && echo y),y)
	DEP_MANAGER ?= Glide
else
	DEP_MANAGER ?= Godeps
endif

ifeq ($(DEP_MANAGER),Godeps)
