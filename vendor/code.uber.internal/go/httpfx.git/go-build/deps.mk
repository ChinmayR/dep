...
DEPS_GOPATH := $(CURDIR)/$(GO_BUILD_ROOT)/.go
...
DEPS_PROGS += $$($1)
$$($1): $(DEPS_GOPATH)/src/gb2/glide.lock $(wildcard $(DEPS_GOPATH)/src/gb2/vendor/$2/*.go)
	$(ECHO_V)GOPATH=$(DEPS_GOPATH) $(GO) build -i -o $$@ gb2/vendor/$2
endef
...
gobuild-clean: ## Internal: Remove build tools (such as glide, golint, etc).
	rm -rf $(DEPS_GOPATH)/.go/bin $(DEPS_GOPATH)/.go/pkg
	rm -rf $(DEPS_PROGS)
